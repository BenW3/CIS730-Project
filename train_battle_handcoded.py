...
def play_a_round(
    ...
    # for i in range(n):
        ...
        ids1 = env.get_agent_id(handles[0])
        ids2 = env.get_agent_id(handles[1])
        # let models infer action in parallel (non-blocking)
        # print(np.asarray(obs[0]).shape)
        actions2 = policy(obs2[0], ids2, env)
        models.infer_action(obs1, ids1, "e_greedy", eps, block=False)

        for i in range(n):
            if i == learner:
                # print('not yet training')
                acts[i] = models.fetch_action()  # fetch actions (blocking)
                # print('not yet training')
            else:
                acts[i] = actions2
            # print('not yet training')
        ...
        for i in range(n):
            ...
            # print('not yet training')
            if train and i == learner:
                # print('trying to train')
                alives = env.get_alive(handles[learner])
                # store samples in replay buffer (non-blocking)
                models.sample_step(rewards, alives, block=False)
            s = sum(rewards)
    ...
    if render:
        ...
        frame_list[0].save('trainH2_'+ str(k+1) + '.gif', save_all=True, append_images=frame_list[1:], duration=0.1, loop=0)
    ...
    if train:
        ...
        # for i in range(n):
        models.train(print_every=1000, block=False)
        # for i in range(n):
        total_loss[0], value[0] = models.fetch_train()
...
if __name__ == "__main__":
    ...
    model_args = {"eval_obs": eval_obs}
    model_args.update(base_args)
    models = ProcessingModel(
            env, handles[0], names[0], 20000, 1000, RLModel, **model_args
        )


    # load if
    savedir = "saveModelHandCoded2"
    if args.load_from is not None:
        start_from = args.load_from
        print("load ... %d" % start_from)
        # for model in models:
        models.load(savedir, start_from)
    else:
        start_from = 0
    ...
    plt.plot(roundplot,adversary, label = 'adversary')
